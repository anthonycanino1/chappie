...
if __name__ == '__main__':
    ...
    print(args.path, args.destination)
    for k, names in enumerate(zip(runtime, threads, ids, energies, jiffies, stacks, activeness)):
        ...
        energy['package'] = energy.groupby('socket')['package'].transform(pd.Series.diff).fillna(0)
        energy['dram'] = energy.groupby('socket')['dram'].transform(pd.Series.diff).fillna(0)
        ...
        non_chappie = thread[thread['thread'] != 'Chaperone']
        chappie = thread[thread['thread'] == 'Chaperone']
        chappie = chappie.sort_values('state', ascending = False).drop_duplicates(['epoch', 'thread'])

        thread = pd.concat([non_chappie, chappie]).sort_values('epoch')
        # print(thread)
        ...
        thread['k_jiffies'] = thread.groupby('tid')['k_jiffies'].transform(pd.Series.diff)
        ...
        non_chappie = id[id['thread'] != 'Chaperone']
        chappie = id[id['thread'] == 'Chaperone']
        chappie = chappie.sort_values('state', ascending = False).drop_duplicates(['epoch', 'thread'])

        id = pd.concat([non_chappie, chappie]).sort_values('epoch')

        thread = pd.merge(id, thread, on = ['epoch', 'thread'], how = 'outer', suffixes = ('', '_'))
        thread['state'] = thread['state'].fillna(thread['state_'])
        activity = activity['activeness'][activity.epoch.isin(thread.epoch)].replace('Infinity', 1).fillna(0).astype(float)
        ...
        ruby_args = ['-c', os.path.realpath(thread_path) , '-h' , os.path.realpath(method_path)]

        opt_path = os.path.dirname(os.path.realpath(__file__))
        ruby_file = os.path.join(opt_path, "opt-align.rb")
        ruby_cmd = ["ruby", ruby_file] + ruby_args

        thread = subprocess.check_output(["ruby", ruby_file] + ruby_args)
