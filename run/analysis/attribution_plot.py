...
def main():
    ...
    for bench in os.listdir(path):
        if bench != 'plots':
            df = pd.read_csv(os.path.join(path, bench, 'summary', 'chappie.component.csv'), index_col = 'benchmark').dropna()
            df['bench'] = bench
    ...
    attribution = pd.concat(attribution)
    attribution = attribution[(attribution.vm == 4) & (attribution.os == 10)]
    attribution = attribution.groupby(['bench', 'socket', 'component'])['package', 'dram'].sum()
    attribution = attribution.unstack('component').reset_index().dropna()

    print(attribution)

    ax = None
    for _, socket in attribution.groupby('socket'):
        # print(socket)
        # sys.exit(0)
        if ax is None:
            ax = socket.plot.bar(
    ...
    # sns.heatmap(mu, cmap = 'Reds', annot = annot, fmt = '', cbar_kws={'label': 'Percent Overhead'})
    plt.xticks(fontsize = 8, rotation = 30)
    plt.xlabel("Benchmark")
    plt.ylabel("Energy (J)")

    plt.savefig(os.path.join(args.benchmark, 'plots', 'attribution_plots.pdf'), bbox_inches = 'tight')
    sys.exit(0)
