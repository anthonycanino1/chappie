...
def main():
    ...
    for bench in benchs:
        ...
        for rate in ['1', '32', '512']: # os.listdir(os.path.join(data_dir, bench)):
            ...
            # cpu_bins = [-1, 20, 41]
            cpu_bins = np.linspace(0, df2.max() + 1, c_size + 1)
            if c_size > 1:
            ...
            # print(pd.concat([df, ref], axis = 1))
            # print(pd.concat([df, ref], axis = 1).fillna(0))

            # print(df.unstack().sum())
            # print(ref.unstack().sum())

            # print(df)
            # print(ref)

            dists.append((int(rate), df.corr(ref))) # pd.concat([df, ref], axis = 1).fillna(0).corr().iloc[0, 1]))
            # continue
            # continue
            # continue
            ...
            df = df / df.sum()
            # df = df.unstack()
            # df = df.reindex(pd.RangeIndex(0, 41)).fillna(0).stack()
            # print(df.unstack())
            continue
            # df = df / df.sum()
        ...
        ref = ref.to_frame().reset_index()
        ref.freq = pd.cut(ref.freq.map(lambda x: (x.left + x.right) / 2), bins = [100, 125, 150, 175, 200, 225, 250, 275, 350])
        # ref.cpu = pd.cut(ref.freq.map(lambda x: (x.left + x.right) / 2), bins = [100, 150, 200, 250, 350])
